<?php
    ...
    class DOCENTES extends CONEXION{
        ...
        public function Insertar_Docente($GET_NOM,$GET_APELLIDO,$GET_USUARIO,$GET_CONTRA){
            $statement = $this->BD->prepare(
                "INSERT INTO USUARIOS (Nombre,Apellido,Usuario,Comtraseña,Perfil,Estado)
                VALUES (:1_Nom, :2_Apellido, :3_Usuario, :4_Contra, 'Docente', 'Activo')"
            );
        ...
        public function Consultar_Docente(){
            $i=0;
            $rows = null;
            $statement = $this->BD->prepare("SELECT * FROM USUARIOS WHERE Perfil = 'Docente'");
            $statement->execute();
        ...
        public function get_Id_Docente($GET_ID){
            $i=0;
            $rows=null;
            $statement = $this->BD->prepare("SELECT * FROM USUARIOS WHERE Perfil = 'Docente' AND Id_Usuario = :1_Id");
        ...
        public function Actu_Docente($GET_ID,$GET_NOMBRE,$GET_APELLIDO,$GET_USUARIO,$GET_CONTRA,$GET_ESTADO){
            $statement = $this->BD->prepare(
                "UPDATE USUARIOS SET
                Nombre = :2_Nombre,
                Apellido = :3_Apellido,
            ...
            $statement->bindParam(':5_Contra',$GET_CONTRA);
        ...
        public function Elim_Docente($GET_ID){
            $statement = $this->BD->prepare("DELETE FROM USUARIOS WHERE Id_Usuario = :1_Id");
